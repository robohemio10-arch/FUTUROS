...
from .trade_link_crosswalk import validate_operational_crosswalk
...
def signal(
    ...
    if (
        row.get("signal_id") != record.signal_id
        or row.get("decision_event_id") != record.event_id
    ):
    ...
    normalized: dict[str, Any] = {
        ...
        "signal_timestamp_utc": observed.isoformat(),
        "decision": record.model_dump(mode="json"),
    }

    crosswalk = validate_operational_crosswalk(
        row.get("operational_crosswalk"),
        v3_decision=record,
    )
    if crosswalk is not None:
        normalized["operational_crosswalk"] = crosswalk
...
def outcome(
    ...
    exact = (
        link.parent_event_id == decision.event_id
        ...
        and link.symbol == decision.symbol
        and link.side == decision.side
        ...
        and type(row.get("trade_id")) is int
        and row["trade_id"] == link.trade_id
    )
    if not exact:
        raise EvidenceError("outcome_causal_identity_mismatch")

    opened = utc(row.get("open_time_utc"))
    closed = utc(row.get("close_time_utc"))
    if not (
        utc(parent["signal_timestamp_utc"])
        <= opened
        == link.execution_timestamp
        < closed
        <= now
    ):
