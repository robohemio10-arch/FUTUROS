"""Build the Daily Candidate Shadow Rule Registry report."""
...
def _load_api() -> tuple[Any, Any, int, float, float]:
...
def build_parser() -> argparse.ArgumentParser:
    _, _, default_support, default_confidence, default_lift = _load_api()
    parser = argparse.ArgumentParser(
        description=(
            "Registra candidate shadow rules apenas como pesquisa em memoria. "
    ...
    parser.add_argument("--min-lift", type=float, default=default_lift)
    return parser


def main(argv: list[str] | None = None) -> int:
    args = build_parser().parse_args(argv)
    build_report, validate_report, _, _, _ = _load_api()
    ...
    if args.json_output:
        ...
        print(f"DAILY_CANDIDATE_SHADOW_RULE_REGISTRY_STATUS={payload['status']}")
        print(f"DAILY_CANDIDATE_SHADOW_RULE_REGISTRY_DECISION={payload['decision']}")
        print(f"DAILY_CANDIDATE_SHADOW_RULE_REGISTRY_JSON={output_text}")
    return exit_code
