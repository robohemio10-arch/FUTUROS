#!/usr/bin/env python3
"""Build the paper autotrain source-key reconciliation report."""
...
from smartcrypto.learning.paper_autotrain_source_key_reconciliation import (  # noqa: E402
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--fail-on-unreconciled-sources",
        action="store_true",
        help="Add an explicit blocker when source-key reconciliation leaves unreconciled groups.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_paper_autotrain_source_key_reconciliation_v1(
        ...
        fail_on_unreconciled_sources=args.fail_on_unreconciled_sources,
    )
    if args.json:
