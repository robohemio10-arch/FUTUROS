"""Build the Daily Candle Coverage and Entry Features report."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Calcula coverage/features apenas com entradas em memoria. "
            "O CLI nao le trades ou candles reais."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json_output:
        ...
        print(f"DAILY_CANDLE_COVERAGE_ENTRY_FEATURES_STATUS={payload['status']}")
        print(f"DAILY_CANDLE_COVERAGE_ENTRY_FEATURES_DECISION={payload['decision']}")
        print(f"DAILY_CANDLE_COVERAGE_ENTRY_FEATURES_JSON={output_text}")
    return exit_code
