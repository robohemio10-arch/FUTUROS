#!/usr/bin/env python3
"""Build the Paper/Master divergence OOS causal attribution report.

The CLI is standalone-safe and research-only. It must not require PYTHONPATH or
an editable install when called as ``python scripts/<name>.py``.
"""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a research-only OOS causal attribution plan for the "
            "Paper/Master divergence without changing runtime or promoting rules."
        )
    )
...
def _safety_error_payload(exc: Exception) -> dict[str, object]:
    return {
        ...
        "reason": "invalid_oos_causal_attribution_structure",
