...
from smartcrypto.ops.dashboard_snapshots.runtime_freshness_producer_contracts import (  # noqa: E402
    audit_runtime_freshness_producer_contracts,
    load_runtime_freshness_producer_contract_inputs,
)


REPORT_PATH = Path("data/reports/runtime_freshness_producer_contracts_audit_v1.json")


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Audit manual contracts for runtime freshness evidence producers."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = audit_runtime_freshness_producer_contracts(
        ...
        **load_runtime_freshness_producer_contract_inputs(project_root),
    )
    ...
    if args.json:
        ...
        print(
            ...
            f"contracts_ready_total={payload['contracts_ready_total']}"
        )
