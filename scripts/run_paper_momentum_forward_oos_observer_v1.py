#!/usr/bin/env python
"""Run the frozen paper momentum forward OOS observer read-only."""
...
from smartcrypto.research.paper_momentum_forward_oos_observer import (  # noqa: E402
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Observe only post-freeze paper trades for the frozen ret12+ret1 "
            "momentum filter against control, without blocking entries or writing runtime."
        )
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = run_momentum_forward_oos_observer(
        args.project_root,
