"""Build the Daily Paper/Master divergence and temporal alignment report."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Calcula divergencia agregada e alignment temporal Paper/Master "
            "somente com entradas em memoria. O CLI nao le dados reais."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json_output:
        ...
        print(f"DAILY_PAPER_MASTER_DIVERGENCE_ALIGNMENT_STATUS={payload['status']}")
        print(f"DAILY_PAPER_MASTER_DIVERGENCE_ALIGNMENT_DECISION={payload['decision']}")
        print(f"DAILY_PAPER_MASTER_DIVERGENCE_ALIGNMENT_JSON={output_text}")
    return exit_code
