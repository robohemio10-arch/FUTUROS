#!/usr/bin/env python3
"""Plan, explicitly apply, or verify the guarded Bitradex legacy append."""
...
from smartcrypto.data.bitradex_ocr_legacy_authorized_append import (  # noqa: E402
    DEFAULT_TRANSITION_CONTRACT,
    apply_authorized_append,
    build_authorized_append_plan,
    verify_authorized_append,
)
from smartcrypto.data.bitradex_ocr_legacy_authorized_append.planner import (  # noqa: E402
    DEFAULT_REPORT_JSON,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    for command in ("plan", "apply", "verify"):
        ...
        subparser.add_argument("--transition-contract", default=str(DEFAULT_TRANSITION_CONTRACT))
        subparser.add_argument("--json", action="store_true")
        if command == "plan":
            subparser.add_argument("--write-report", action="store_true")
            subparser.add_argument("--output-json", default=str(DEFAULT_REPORT_JSON))
            subparser.add_argument("--output-markdown", default=str(DEFAULT_REPORT_MARKDOWN))
        elif command == "apply":
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "apply":
        ...
        report = verify_authorized_append(
            project_root=args.project_root,
            transition_contract_path=args.transition_contract,
        )
    else:
        report = build_authorized_append_plan(
            project_root=args.project_root,
            transition_contract_path=args.transition_contract,
