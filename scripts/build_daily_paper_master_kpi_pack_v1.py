"""Build the Daily Paper/Master KPI pack payload."""
...
def _load_api() -> tuple[Any, Any]:
    ...
    return build_daily_paper_master_kpi_pack, validate_daily_paper_master_kpi_pack


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Calcula um KPI pack agregado Paper/Master apenas com entradas em "
            "memoria. Por default nao carrega trades reais nem escreve runtime."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    build_pack, validate_pack = _load_api()
    project_root = Path(args.project_root).expanduser().resolve()
    payload = build_pack(project_root)
    ...
    if args.output and not args.no_write:
        ...
        if path_error is not None:
            ...
            payload["validation_errors"] = validate_pack(payload)
            exit_code = 1
        else:
    ...
    if args.json_output:
        ...
        print(f"DAILY_PAPER_MASTER_KPI_PACK_STATUS={payload['status']}")
        print(f"DAILY_PAPER_MASTER_KPI_PACK_DECISION={payload['decision']}")
        print(f"DAILY_PAPER_MASTER_KPI_PACK_JSON={output_text}")
    return exit_code
