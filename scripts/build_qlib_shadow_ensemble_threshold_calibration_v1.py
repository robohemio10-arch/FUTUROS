#!/usr/bin/env python3
"""Build research-only Qlib/IA Shadow ensemble threshold calibration."""
...
from smartcrypto.learning.qlib_shadow_ensemble_threshold_calibration.calibrator import (  # noqa: E402
    DEFAULT_REPORT_JSON,
    DEFAULT_REPORT_MD,
...
def main() -> int:
    ...
    report = build_qlib_shadow_ensemble_threshold_calibration_v1(
        project_root=root,
        write=write_requested,
    )
    report_json = resolve(root, args.report_json, DEFAULT_REPORT_JSON)
    report_md = resolve(root, args.report_markdown, DEFAULT_REPORT_MD)
    report["output_paths"] = {"json": str(report_json), "markdown": str(report_md)}
    if write_requested:
