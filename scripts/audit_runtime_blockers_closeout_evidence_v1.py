...
from smartcrypto.ops.dashboard_snapshots.runtime_blockers_closeout_evidence import (  # noqa: E402
    audit_runtime_blockers_closeout_evidence,
    load_closeout_audit_inputs,
)


REPORT_PATH = Path("data/reports/runtime_blockers_closeout_evidence_audit_v1.json")


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Audit runtime blocker closeout against materialized read-only evidence."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    inputs = load_closeout_audit_inputs(project_root)
    payload = audit_runtime_blockers_closeout_evidence(
    ...
    if args.json:
        ...
        print(
            ...
            f"closeout_allowed={str(payload['closeout_allowed']).lower()}"
        )
