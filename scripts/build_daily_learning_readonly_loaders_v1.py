"""Build the Daily Learning read-only loader report."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Inspeciona metadata read-only das fontes Daily Learning sem "
            "carregar linhas, calcular KPIs ou escrever runtime por default."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json_output:
        ...
        print(f"DAILY_LEARNING_READONLY_LOADERS_STATUS={payload['status']}")
        print(f"DAILY_LEARNING_READONLY_LOADERS_DECISION={payload['decision']}")
        print(f"DAILY_LEARNING_READONLY_LOADERS_JSON={output_text}")
    return exit_code
