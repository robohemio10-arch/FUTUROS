...
def make_rows(
    *,
    selected_oos_pnl: float = 2.0,
    selected_every: int = 1,
    selected_oos_count: int = 60,
    ...
    for index in range(240):
        is_oos = index >= oos_start_index
        oos_offset = index - oos_start_index

        selected = (
            is_oos
            and oos_offset < selected_oos_count
            and oos_offset % selected_every == 0
        )
...
def test_oos_edge_passes_with_profitable_selected_trades(
    tmp_path: Path,
) -> None:
    ...
    assert (
        report["decision"]
        == "OOS_ECONOMICALLY_PROMISING_RESEARCH_ONLY"
    )
    assert report["oos_trade_count"] == 72
    assert report["oos_selected_trade_count"] == 60
    assert report["chronological_boundary_valid"] is True
    assert report["oos_selected_metrics"]["net_pnl_total"] > 0
    assert report["oos_selected_metrics"]["expectancy"] > 0
    assert report["oos_selected_metrics"]["profit_factor"] == "inf"
    assert report["oos_expectancy_uplift"] > 0
    assert report["blockers"] == []


def test_oos_requires_enough_selected_trades(
    tmp_path: Path,
) -> None:
    report = build_paper_autolearning_economic_oos_validation_v1(
        project_root=tmp_path,
...
def test_oos_rejects_negative_selected_economics(
    tmp_path: Path,
) -> None:
    report = build_paper_autolearning_economic_oos_validation_v1(
        project_root=tmp_path,
...
def test_oos_fails_closed_on_invalid_time(
    tmp_path: Path,
) -> None:
    rows = make_rows()
...
def test_oos_preserves_operational_safety(
    tmp_path: Path,
) -> None:
    report = build_paper_autolearning_economic_oos_validation_v1(
        project_root=tmp_path,
