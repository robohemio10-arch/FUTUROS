...
PRICE_GUARDS: dict[str, tuple[float, float]] = {
    "BTCUSDT": (10_000.0, 300_000.0),
    "ETHUSDT": (500.0, 20_000.0),
}

MAX_TICKER_AGGREGATED_RANGE_RATIO: dict[str, float] = {
    "15s": 1.02,
    "1m": 1.05,
    "3m": 1.08,
    "5m": 1.10,
...
class DataProcessor:
    ...
    def _is_valid_candle(self, candle: Candle) -> bool:
        ...
        if candle.timeframe not in TIMEFRAME_SECONDS:
            return False
        if not all(self._is_price_inside_symbol_guard(candle.symbol, float(x)) for x in prices):
            return False
        if not self._is_ticker_aggregated_range_sane(candle):
            return False
        return True
    ...
    def _extract_ticker_updates(self, obj: Any, captured_at: datetime, context_symbol: str | None) -> Iterable[dict[str, Any]]:
        """Yield strict public ticker updates for configured symbols.

        Safety policy:
        - Do not infer symbol from arbitrary JSON text.
        - Do not apply page context_symbol to nested public multi-market payloads.
        - Use captured_at as the tick timestamp; many ticker payloads include
          historical/open-time fields that are not the tick event time.
        - Reject prices outside institutional symbol guards before aggregation.
        """
        if isinstance(obj, dict):
            symbol = self._ticker_symbol_from_dict(obj)
            price = self._ticker_price_from_dict(obj)

            if symbol and price is not None and symbol in self.config.symbols:
                if self._is_price_inside_symbol_guard(symbol, float(price)):
    ...
    def _ticker_symbol_from_dict(self, item: dict[str, Any]) -> str | None:
        """Return symbol only from explicit symbol-like fields.

        Previous versions searched the whole JSON document for BTC/ETH aliases.
        That caused multi-asset ticker payloads to be attributed to BTCUSDT or
        ETHUSDT while the selected price belonged to another instrument.
        """
        symbol_value = self._first_present(
            item,
            (
                "symbol", "s", "pair", "market", "contract", "contractName",
                "instrument", "instId", "name", "code", "topic", "channel",
                "ch", "stream",
            ),
        )
        if symbol_value is None:
            return None
        return self._normalize_symbol(symbol_value)

    def _ticker_price_from_dict(self, item: dict[str, Any]) -> float | None:
        keys = (
            "lastPrice", "last_price", "last", "latestPrice", "latest_price",
            "tradePrice", "trade_price", "price", "p", "markPrice",
            "mark_price", "indexPrice", "index_price", "fairPrice",
            "close", "closePrice", "c",
        )
        for key in keys:
            if key in item:
    ...
    def _ticker_volume_from_dict(self, item: dict[str, Any]) -> float | None:
        for key in ("volume", "vol", "v", "amount", "quantity", "q", "quoteVolume", "quote_volume", "baseVolume", "turnover"):
            if key in item:
                value = self._to_float(item.get(key))
                if value is not None and value >= 0:
                    return value
    ...
    def _is_price_inside_symbol_guard(self, symbol: str, price: float) -> bool:
        guard = PRICE_GUARDS.get(symbol)
        if not guard:
            return False
        low, high = guard
        return low <= float(price) <= high

    def _is_ticker_aggregated_range_sane(self, candle: Candle) -> bool:
        if "ticker_aggregated" not in candle.transport:
            return True
        if candle.low <= 0:
            return False
        max_ratio = MAX_TICKER_AGGREGATED_RANGE_RATIO.get(candle.timeframe, 1.10)
        return (candle.high / candle.low) <= max_ratio
